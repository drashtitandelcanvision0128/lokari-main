...
import { loginWithAdminCredentials } from '@/lib/auth/api';
...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { user, token } = await loginWithAdminCredentials({
  ...
  return (
    <div
      className={`min-h-screen flex items-center justify-center ${isDark ? 'bg-gray-900' : 'bg-[#f9f9f7]'} px-4`}
    >
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          <Link
            href="/"
            className="inline-flex items-center gap-3 text-xl font-bold font-headline mb-6"
          >
            <img src="/AgriwareLogo.svg" alt="Lokhari Logo" className="w-8 h-8" />
            <span className={isDark ? 'text-white' : 'text-[#0b5d68]'}>Lokhari</span>
          </Link>
          <h1
            className={`font-headline text-3xl font-bold mb-2 ${isDark ? 'text-white' : 'text-[#0b5d68]'}`}
          >
            Admin Portal
          </h1>
        ...
        <div className={`${isDark ? 'bg-gray-800' : 'bg-white'} rounded-2xl shadow-xl p-8`}>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label
                htmlFor="email"
                className={`block text-sm font-medium mb-2 ${isDark ? 'text-gray-200' : 'text-[#0b5d68]'}`}
              >
                Email Address
              </label>
              <div className="relative">
                ...
                />
                <span
                  className={`absolute right-3 top-1/2 transform -translate-y-1/2 material-symbols-outlined ${
            ...
            {error && (
              <div
                className={`p-3 rounded-lg text-sm ${isDark ? 'bg-red-900/50 text-red-200' : 'bg-red-50 text-red-600'}`}
              >
                {error}
              </div>
            )}
          ...
          {/* Demo Credentials */}
          <div
            className={`mt-6 p-4 rounded-lg ${isDark ? 'bg-gray-700' : 'bg-gray-50'} border ${isDark ? 'border-gray-600' : 'border-gray-200'}`}
          >
            <p
              className={`text-sm font-medium mb-2 ${isDark ? 'text-gray-200' : 'text-[#0b5d68]'}`}
            >
              Demo Credentials:
            </p>
            <div className={`text-xs space-y-1 ${isDark ? 'text-gray-300' : 'text-[#666666]'}`}>
              <p>
                <strong>Email:</strong> admin@lokhari.com
              </p>
              <p>
