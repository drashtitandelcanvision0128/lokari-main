...
export default function KYCPage() {
  ...
  useEffect(() => {
    ...
    // Load existing KYC status from registration service
  ...
  const handleKYCSubmit = async () => {
    ...
    try {
      // Submit KYC using registration service
  ...
  const handleSkipForNow = () => {
    // Get dashboard URL from registration service
    const currentUser = registrationService.getCurrentUser()
    const dashboardUrl = currentUser ? registrationService.getDashboardUrl(currentUser.role) : '/dashboard'
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'not_started': return 'text-[#666666]'
      case 'submitted': return 'text-[#e89151]'
      case 'under_review': return 'text-[#2eb5c2]'
      case 'verified': return 'text-[#0b5d68]'
      case 'rejected': return 'text-[#d55b39]'
  ...
  const getStatusBg = (status: string) => {
    switch (status) {
      case 'not_started': return 'bg-[#f0f0f0]'
      case 'submitted': return 'bg-[#ffe8e0]'
      case 'under_review': return 'bg-[#a5dce4]'
      case 'verified': return 'bg-[#c1e8ed]'
      case 'rejected': return 'bg-[#ffdad6]'
        ...
        <div className="bg-white rounded-xl shadow-sm border border-[#e0e0e0] p-8">
          ...
          <div className={`mb-6 p-4 rounded-lg ${getStatusBg(kycStatus.status)}`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                <span className={`material-symbols-outlined ${getStatusColor(kycStatus.status)} mr-2`}>
                  {kycStatus.status === 'verified' ? 'verified' : 
                   kycStatus.status === 'under_review' ? 'pending' :
                   kycStatus.status === 'submitted' ? 'upload' :
                   kycStatus.status === 'rejected' ? 'error' : 'hourglass_empty'}
                </span>
                <div>
                  ...
                  {!showOTP ? (
                    <form onSubmit={handleAadhaarSubmit} className="space-y-3">
                      <Input
                        ...
                        onChange={(e) => setKycData({ ...kycData, aadhaarNumber: e.target.value.replace(/\D/g, '') })}
                    ...
                    {config.requiredDocuments.map((doc) => (
                      doc !== 'aadhaar' && (
                        <div key={doc}>
                          ...
                          />
                          {kycData[doc as keyof LocalKYCData] && typeof kycData[doc as keyof LocalKYCData] !== 'string' && (
          ...
          {kycStatus.status === 'verified' && (
            <TransitionWrapper show={true} animation="scaleIn">
              ...
              <p className="text-[#0b5d68] font-medium">
            ...
            <TransitionWrapper show={true} animation="slideIn">
              ...
              <div className="text-center space-y-3">
                <div className="w-16 h-16 bg-[#ffdad6] rounded-full flex items-center justify-center mx-auto">
                  <span className="material-symbols-outlined text-[#d55b39] text-2xl">
                    error
                  </span>
                </div>
                <p className="text-[#d55b39] font-medium">
                  KYC Verification Failed
                </p>
                <p className="text-[#666666]">
                  {kycStatus.rejectionReason || 'Your KYC verification was rejected. Please review and resubmit.'}
                </p>
              </div>
              <div className="flex gap-3">
                <Button
